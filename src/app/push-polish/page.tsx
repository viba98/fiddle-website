...
export default function Home() {
  ...
  // Access React global hook
  const toggleModalWithHook = () => {
    console.log('Attempting to toggle modal with hook...');
    const devTools = (window as any).__REACT_DEVTOOLS_GLOBAL_HOOK__;
    console.log('DevTools:', devTools);
    if (devTools) {
      const fiber = devTools.getFiberRoots(1).values().next().value;
      console.log('Root Fiber:', fiber);
      
      // Traverse to find our component
      const findHomeComponent = (fiber: any): any => {
        if (fiber.elementType?.name === 'Home') {
          return fiber;
        }
        if (fiber.child) {
          const result = findHomeComponent(fiber.child);
          if (result) return result;
        }
        if (fiber.sibling) {
          const result = findHomeComponent(fiber.sibling);
          if (result) return result;
        }
        return null;
      };

      const homeComponent = findHomeComponent(fiber.current);
      console.log('Home Component:', homeComponent);

      if (homeComponent) {
        // Find the showOnboarding state hook - it should be the 7th boolean hook
        let currentHook = homeComponent.memoizedState;
        let hookIndex = 0;
        let booleanHookCount = 0;
        
        while (currentHook) {
          console.log('Hook', hookIndex, ':', currentHook.memoizedState);
          
          if (typeof currentHook.memoizedState === 'boolean') {
            console.log('Found boolean state', booleanHookCount, ':', currentHook.memoizedState);
            
            // showOnboarding is the 5th boolean hook (0-based index: 4)
            if (booleanHookCount === 4) {
              if (currentHook.queue?.dispatch) {
                console.log('Found showOnboarding dispatch');
                currentHook.queue.dispatch((prev: boolean) => !prev); // Toggle the state
        ...
        console.log('Could not find Home component');
      }
    } else {
  ...
  // Test the hook in useEffect
  useEffect(() => {
    if (typeof window !== 'undefined') {
      console.log('Testing modal hook...');
      setTimeout(() => {
        toggleModalWithHook();
